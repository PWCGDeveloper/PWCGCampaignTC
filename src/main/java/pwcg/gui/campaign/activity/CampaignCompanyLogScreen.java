...
public class CampaignCompanyLogScreen extends ImageResizingPanel implements ActionListener
{
	...
	public CampaignCompanyLogScreen (int logsForCompanyId)
	{        
        super("");
	...
	{
        String imagePath = UiImageResolver.getImage(ScreenIdentifier.CampaignCompanyLogScreen);
        this.setImageFromName(imagePath);

        CampaignCompanyLogSummaryPageBuilder summaryPageBuilder = new CampaignCompanyLogSummaryPageBuilder(campaign, logsForCompanyId);
        Map<Integer, StringBuffer> summaryPages = summaryPageBuilder.buildSummaryPage();
        pages.putAll(summaryPages);

        CampaignCompanyLogDetailPageBuilder detailPageBuilder = new CampaignCompanyLogDetailPageBuilder(logsForCompanyId);
	...
	private JPanel makeLogLeftPanel() throws PWCGException  
	{
        ...
        JButton finishedButton = PWCGButtonFactory.makeTranslucentMenuButton("Finished Reading", "CompanyLogFinished", "Leave Company Log", this);
	...
	{
        String imagePath = UiImageResolver.getImage(ScreenIdentifier.OpenCompanyLog);
	...
	public void actionPerformed(ActionEvent ae)
	{
		try
		{
            ...
            if (action.equalsIgnoreCase("CompanyLogFinished"))
            {
                campaign.write();                
